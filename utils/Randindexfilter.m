function [ind_folds,num_c] = Randindexfilter(num_sample,per,num_view)  %num_c是完整的样本数
    %input:per为缺失数据百分比
    %outout:一个num_sample*num_view的指示矩阵，数值为0的位置为缺失数据index
    ind_folds = [];
    record = zeros(1,num_sample);   %用于记录每一列被去除的次数
    for v=1:num_view
        empty_temp = randperm(num_sample,round(num_sample*per));
        for j = 1:round(num_sample*per)
